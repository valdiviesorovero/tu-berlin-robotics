...
protected:
  ...
  void Pack2B( char *text, uint16_t data );
  void Pack4B( char *text, uint32_t data );
  void Pack8B( char *text, double data );

  int  Unpack( char *data, int num, int size ) const;
  char      Unpack1B( const char *text ) const;
  uint16_t  Unpack2B( const char *text ) const;
  uint32_t  Unpack4B( const char *text ) const;
...
//  int WriteLong( const long *data, int num )   { return Pack((const char *)data,num,4); }
  ...
  int  WriteInt( int32_t value )  { return Pack((const char *)(&value),1,4); }
  ...
  int ReadShort( int16_t *data, int num ) const   { return Unpack((char *)data,num,2); }
  int ReadInt( int32_t *data, int num ) const       { return Unpack((char *)data,num,4); }
//  int ReadLong( long *data, int num ) const     { return Unpack((char *)data,num,4); }
  ...
  int32_t   ReadInt()   const { 
            int32_t val=-1;
            Unpack((char *)&val,1,4);
            //static int cntr = 0; if (cntr++ == 0)  std::cerr << "AHA: " << val;
            return val; 
  }
...
inline void AMsg::Pack8B( char *text, double data )
{
  ...
  TODO: the lower code was commented out - but it's the one that works on linux..
  */
  *(double *)text = data;
}
...
inline uint16_t AMsg::Unpack2B( const char *text ) const
{
  return ntohs(*(const uint16_t *)text);
  //return *(const short *)text;
}

inline uint32_t AMsg::Unpack4B( const char *text ) const
{
...
inline double AMsg::Unpack8B( const char *text ) const
{
  ...
  /*
  TODO: the upper code was commented out - but it's the one that works on linux..
  RemapDouble x;
  if (m_fEndian == PR_LITTLE_ENDIAN)
  {
    x.s.l = Unpack4B(text);
